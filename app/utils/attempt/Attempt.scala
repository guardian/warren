...
case class Attempt[A] private (underlying: Future[Either[Failure, A]]) {
  ...
  // Blocks then turns into an option - probably bad lmao
  def toOption()(implicit ec: ExecutionContext): Option[A] = Await.result(underlying, Duration.Inf) match {
    case Left(_) => None
    case Right(v) => Some(v)
  }
...
object Attempt {
  ...
  def fromOption[A](optA: Option[A], ifNone: => Attempt[A]): Attempt[A] = optA match {
